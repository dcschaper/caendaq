#!/bin/sh
# ****************************************************************
# Script written for CAEN V1724 DPP-PHA firmware
# Modifying existing caen_daq script with registers from User Manual UM5407
# ************************************************
# v0  initial version			2017-01-25
# At the end of this script is a FAQ explaining some of the details of how it works.

BASE=3210 #this is the base address for the board. To find/set it, look at the back side of the caen card and find the numbers that the yellow plastic arrows are pointing to

N_TRIG=$1 #first positional parameter (First one you enter when you run the program)
echo -n "$N_TRIG triggers " >/dev/stderr #this dumps error information to stderr
...
N_PAIRS=$3
if [[ -z $N_PAIRS ]]; then N_PAIRS=3999; fi #number of pairs is the third positional parameter, but if you leave it blank it defaults to 3999 pairs
...
##############################
# configure and set up the DAQ
##############################
#Note: All registers are in numerical order to make referencing user manual easier

WRITE_REGISTER EF24 0		# Reset the board
WRITE_REGISTER EF1C 1		# Aggregate number per BLT 
WRITE_REGISTER EF00 10		# VME Control Register, Enable BERR (Bus Error)
WRITE_REGISTER 800C `printf %x $BUF`		# Buffer Organization (Num Buffers = 2^N)
WRITE_REGISTER 811C 1		# Front Panel I/O Control: TTL
WRITE_REGISTER 810C C0000000	# Global Trigger Mask--NEED TO ASK CRAWFORD ABOUT THIS ONE****************
...
# Acquisition Control
WRITE_REGISTER 8100 04  	# Acquisition Control (Do I want 'first trigger controlled?')
...
# Stop acquistion
WRITE_REGISTER		#WHAT REGISTER SHOULD GO HERE??
